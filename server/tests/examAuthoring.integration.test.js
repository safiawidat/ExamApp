...
const createQuestion = async (authorization, examId, label) => {
  const response = await request(app)
    .post(`/api/exams/${examId}/questions`)
    .set('Authorization', authorization)
    .send({
      question_type: 'true_false',
      prompt: `Test-only ${label} question`,
      points: 2,
      correct_answer: true,
    });
...
describe('lecturer exam publication', () => {
  test('publishes an owned draft with a question and returns the public exam shape', async () => {
    const examType = await createExamType(primaryAuthorization, 'publish_success_type');
    const exam = await createExam(
      primaryAuthorization,
      examType.id,
      'publish_success',
    );
    await createQuestion(primaryAuthorization, exam.id, 'publish_success');

    const response = await request(app)
      .post(`/api/exams/${exam.id}/publish`)
      .set('Authorization', primaryAuthorization);
    ...
    expect(Number.isNaN(Date.parse(response.body.published_at))).toBe(false);

    const persisted = await pool.query(
      `SELECT status, published_at,
              (SELECT COUNT(*)::INTEGER FROM questions WHERE exam_id = $1)
                AS question_count
       FROM exams
       WHERE id = $1`,
      [exam.id],
  ...
  test('rejects a zero-question draft without partially publishing it', async () => {
    const examType = await createExamType(primaryAuthorization, 'publish_empty_type');
    const exam = await createExam(primaryAuthorization, examType.id, 'publish_empty');
    ...
    expect(response.body.error).toMatch(/at least one question/i);

    const persisted = await pool.query(
      'SELECT status, published_at FROM exams WHERE id = $1',
      [exam.id],
    );
  ...
  test('rejects repeat publication and preserves the original publication state', async () => {
    const examType = await createExamType(primaryAuthorization, 'publish_repeat_type');
    const exam = await createExam(primaryAuthorization, examType.id, 'publish_repeat');
    await createQuestion(primaryAuthorization, exam.id, 'publish_repeat');
    ...
    expect(second.body.error).toMatch(/already published/i);

    const persisted = await pool.query(
      'SELECT status, published_at FROM exams WHERE id = $1',
      [exam.id],
    );
    expect(persisted.rows[0].status).toBe('published');
    expect(persisted.rows[0].published_at.toISOString()).toBe(first.body.published_at);
  });

  test('hides foreign and nonexistent exams during publication', async () => {
    const examType = await createExamType(primaryAuthorization, 'publish_owner_type');
    const exam = await createExam(primaryAuthorization, examType.id, 'publish_owner');
    await createQuestion(primaryAuthorization, exam.id, 'publish_owner');

    const foreign = await request(app)
      .post(`/api/exams/${exam.id}/publish`)
      .set('Authorization', secondaryAuthorization);
    const nonexistent = await request(app)
      .post('/api/exams/2147483647/publish')
      .set('Authorization', primaryAuthorization);

    expect(foreign.status).toBe(404);
    expect(nonexistent.status).toBe(404);
    expect(foreign.body).toEqual({ error: 'Exam not found.' });
    expect(nonexistent.body).toEqual({ error: 'Exam not found.' });
  ...
  test('enforces authentication, lecturer authorization, and valid exam IDs', async () => {
    const examType = await createExamType(primaryAuthorization, 'publish_access_type');
    const exam = await createExam(primaryAuthorization, examType.id, 'publish_access');
    ...
    const invalidId = await request(app)
      .post('/api/exams/not-an-id/publish')
      .set('Authorization', primaryAuthorization);
    ...
    expect(invalidId.body.error).toMatch(/positive integer/i);
  });

  test('rejects every non-empty or non-object publication payload', async () => {
    const examType = await createExamType(primaryAuthorization, 'publish_payload_type');
    const exam = await createExam(primaryAuthorization, examType.id, 'publish_payload');
    const responses = [
      await request(app)
        .post(`/api/exams/${exam.id}/publish`)
        .set('Authorization', primaryAuthorization)
  ...
  test('keeps published exam and question content immutable', async () => {
    const examType = await createExamType(primaryAuthorization, 'publish_immutable_type');
    const exam = await createExam(
      primaryAuthorization,
      examType.id,
      'publish_immutable',
    ...
    const responses = [
      await request(app)
        .patch(`/api/exams/${exam.id}`)
        .set('Authorization', primaryAuthorization)
        .send({ title: 'Published title must not change' }),
      await request(app)
        .delete(`/api/exams/${exam.id}`)
        .set('Authorization', primaryAuthorization),
      await request(app)
        .post(`/api/exams/${exam.id}/questions`)
        .set('Authorization', primaryAuthorization)
        .send({
          question_type: 'true_false',
          prompt: 'Published exam must reject this question',
          points: 1,
          correct_answer: false,
        }),
      await request(app)
        .patch(`/api/exams/${exam.id}/questions/${question.id}`)
        .set('Authorization', primaryAuthorization)
        .send({ prompt: 'Published prompt must not change' }),
      await request(app)
        .delete(`/api/exams/${exam.id}/questions/${question.id}`)
        .set('Authorization', primaryAuthorization),
      await request(app)
        .put(`/api/exams/${exam.id}/questions/reorder`)
        .set('Authorization', primaryAuthorization)
        .send({ question_ids: [question.id] }),
    ];
    ...
    const persisted = await pool.query(
      `SELECT e.title, e.status, e.published_at,
              COUNT(q.id)::INTEGER AS question_count,
              MIN(q.text) AS question_prompt
       FROM exams e
